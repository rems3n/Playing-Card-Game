...
import { AIDifficulty, GameEventType, GamePhase, GameType } from '@card-game/shared-types';
...
function memoryStore() {
  ...
  return {
    save: vi.fn(async (id: string, value: SerializedGame) => { data.set(id, structuredClone(value)); }),
...
describe('GameService bot scheduler', () => {
  it.each([GameType.Euchre, GameType.SevenSix])('finishes an all-bot %s game across round and bidding boundaries', async (gameType) => {
    ...
    expect(events.filter((e) => e.type === GameEventType.GameEnded)).toHaveLength(1);
    expect(new Set(events.map((e) => e.sequenceNum)).size).toBe(events.length);
    expect((await store.load(id))!.engineData.state).toEqual(room.engine.getState());
  });
  ...
  it('stops at a human turn and rejects a stranger joining an active game', async () => {
    ...
    await expect(service.joinGame(id, 'stranger', 'Alex')).rejects.toThrow('already started');
  ...
  it('cancels a delayed bot action when its game is removed', async () => {
    ...
    const service = new GameService(memoryStore(), () => { entered(); return new Promise<void>((resolve) => { release = resolve; }); });
    const id = service.createGame(GameType.Euchre);
    await service.fillWithAI(id, AIDifficulty.Beginner);
    await service.startGame(id);
    const room = (await service.getRoom(id))!;
  ...
  it('restores one shared engine for simultaneous requests', async () => {
    ...
    const [one, two] = await Promise.all([restored.getRoom(id), restored.getRoom(id)]);
    expect(one).toBe(two);
  ...
  it('only replaces a valid disconnected player and counts connected humans', async () => {
    ...
    await expect(service.replaceWithAI(id, 0)).rejects.toThrow('still connected');
