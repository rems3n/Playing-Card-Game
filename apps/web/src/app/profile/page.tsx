...
export default function ProfilePage() {
  ...
  const [ratingHistories, setRatingHistories] = useState<
    Record<string, Array<{ date: string; rating: number }>>
  >({});
  ...
  useEffect(() => {
    ...
    async function syncAndLoad() {
      try {
        ...
        await apiFetch(`${SERVER_URL}/api/users/sync`, {
          method: "POST",
        ...
        const res = await apiFetch(
          `${SERVER_URL}/api/users/me?email=${encodeURIComponent(email)}`,
        );
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await apiFetch(`${SERVER_URL}/api/users/me`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const usernameHint = {
    ...
    invalid: {
      text: "Letters, numbers, underscores only (3-30 chars)",
      color: "text-[var(--accent-red)]",
    },
  }[usernameStatus];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(profile?.ratings ?? []).map((r) => (
            ...
            >
              ...
              <RatingChart
                data={ratingHistories[r.gameType] ?? []}
                height={100}
              />
            </div>
          ))}
        ...
        <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-subtle)] p-5">
          {Object.keys(profile?.gameStats ?? {}).length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {Object.entries(profile?.gameStats ?? {}).map(
                ([gameType, stats]) => (
                  <div key={gameType}>
