...
export function FriendsList() {
  ...
  async function loadFriends() {
    try {
      const res = await apiFetch(
        `${SERVER_URL}/api/friends?email=${encodeURIComponent(email!)}`,
      );
  ...
  async function loadRequests() {
    try {
      const res = await apiFetch(
        `${SERVER_URL}/api/friends/requests?email=${encodeURIComponent(email!)}`,
      );
      const data = await res.json();
  ...
  async function sendFriendRequest() {
    ...
    setAddStatus(null);
    try {
      const res = await apiFetch(`${SERVER_URL}/api/friends/request`, {
        method: "POST",
  ...
  async function respondToRequest(friendshipId: string, accept: boolean) {
    if (!email) return;
    try {
      await apiFetch(`${SERVER_URL}/api/friends/respond`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
              ...
              friends.map((f) => (
                ...
                >
                  {f.avatarUrl ? (
                    <img
                      src={
                        f.avatarUrl.startsWith("/")
                          ? `${SERVER_URL}${f.avatarUrl}`
                          : f.avatarUrl
                      }
                      alt=""
          ...
          <div>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[var(--text-secondary)] text-sm">
                  @
                </span>
                <input
                  type="text"
            ...
            {addStatus && (
              <p
                className={`text-xs mt-2 ${addStatus.includes("sent") ? "text-[var(--accent-green)]" : "text-[var(--accent-red)]"}`}
              >
                {addStatus}
              </p>
            )}
